...
FastAPI application serving three architectural layers: routes (HTTP endpoints), services (business logic), and models (request/response schemas). Integrates LangGraph workflows (chat, ask, source_chat), Postgres persistence, and AI providers via Esperanto.
...
- **sources_service.py**: Ingests files/URLs (content_core), extracts text, vectorizes, saves to Postgres
...
- **Postgres transactions**: Services use repo_query, repo_create, repo_upsert from database layer
...
- `open_notebook.database`: Postgres repository functions (repo_query, repo_create, repo_upsert)
...
- `open_notebook.jobs`: Job queue for async operations (podcast generation)
...
- **Database: Use test database** (separate Postgres instance or mock repo_query)

---

## Credential Management (API Configuration UI)

The Credential Management system enables users to configure AI provider credentials through the UI instead of environment variables. Keys are stored securely in Postgres (encrypted via Fernet) with database-first fallback to environment variables.
...
1. API endpoint saves keys to `Credential` records (encrypted in Postgres)
